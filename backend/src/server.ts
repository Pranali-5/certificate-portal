...
const auth = (req: AuthedRequest, _res: Response, next: NextFunction) => {
  try {
    ...
    next(error instanceof ApiError ? error : new ApiError(401, 'Invalid session', 'UNAUTHORIZED'));
  }
};
...
const applicationSchema = z.object({
  ...
  dob: z.coerce.date(),
  regNumber: z.string().trim().min(2).max(40),
...
app.post(
  ...
  asyncRoute(async (req: AuthedRequest, res) => {
    ...
    const application = await prisma.application.create({
      data: { ...input, refNumber: refFor(), userId: req.userId! },
    });
...
app.delete(
  ...
  asyncRoute(async (req: AuthedRequest, res) => {
    const application = await prisma.application.findFirst({
      where: {
        userId: req.userId,
        OR: [{ id: req.params.id as string }, { refNumber: req.params.id as string }],
      },
      include: { documents: true },
    });
...
app.patch(
  ...
  asyncRoute(async (req: AuthedRequest, res) => {
    ...
    if (application.status === 'COMPLETED')
      throw new ApiError(409, 'Submitted applications cannot be edited', 'APPLICATION_LOCKED');
    res.json(await prisma.application.update({ where: { id: application.id }, data: input }));
  }),
);

const documentSchema = z.object({
  type: z.enum(['ID_PROOF', 'DEGREE_CERTIFICATE']),
...
app.post(
  '/api/applications/:id/documents/presign',
  auth,
  asyncRoute(async (req: AuthedRequest, res) => {
...
app.post(
  ...
  asyncRoute(async (req: AuthedRequest, res) => {
    ...
    const application = await applicationForUser(req.params.id as string, req.userId);
    if (!input.s3Key.startsWith(`users/${req.userId}/applications/${application.id}/`))
      throw new ApiError(400, 'Invalid document location', 'INVALID_DOCUMENT_KEY');
    if (!bucket)
      throw new ApiError(503, 'File storage is not configured', 'STORAGE_NOT_CONFIGURED');
    try {
      ...
      if (
        ...
        throw new ApiError(400, 'Only PDF files up to 5MB are accepted', 'INVALID_FILE');
      const document = await prisma.document.upsert({
        where: { applicationId_type: { applicationId: application.id, type: input.type } },
        create: {
      ...
      throw new ApiError(400, 'The uploaded file could not be verified', 'UPLOAD_NOT_FOUND');
    }
  }),
);

app.get(
  '/api/applications/:id/documents/:type/download',
  auth,
  asyncRoute(async (req: AuthedRequest, res) => {
    ...
    const url = await getSignedUrl(
      ...
      new GetObjectCommand({
        Bucket: bucket,
        Key: document.s3Key,
        ResponseContentDisposition: `attachment; filename="${document.originalName.replace(/"/g, '')}"`,
      }),
      { expiresIn: 300 },
    );
...
app.post(
  ...
  asyncRoute(async (req: AuthedRequest, res) => {
    ...
    if (!application) throw new ApiError(404, 'Application not found', 'NOT_FOUND');
    if (
      !application.documents.some((document) => document.type === 'ID_PROOF') ||
      !application.documents.some((document) => document.type === 'DEGREE_CERTIFICATE')
    )
      throw new ApiError(
        400,
        'Upload both ID proof and degree certificate before submitting',
        'DOCUMENTS_REQUIRED',
      );
    if (application.status === 'COMPLETED' && application.certificateKey) {
    ...
    if (application.certificateKey?.endsWith('-ist.pdf') && bucket) {
      res.json({
        url: await getSignedUrl(
          s3,
          new GetObjectCommand({ Bucket: bucket, Key: application.certificateKey }),
          { expiresIn: 300 },
        ),
      });
      return;
